...
# --- environment & logging ---
# Lokal çalışırken .env yükler; Azure App Service'de App Settings kullanılır
load_dotenv()
...
def get_db_connection():
    ...
    ENV:
      ...
      ODBC_DRIVER (opsiyonel; varsayılan: ODBC Driver 17 for SQL Server)
    """
    try:
...
async def upload_and_analyze_document(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Bazı ortamlar image_data ister, bazıları image_url ile çalışır.
        # Önce image_data ile dener, TypeError alırsak image_url ile deneriz.
        try:
            ...
            # Eski imza image_data beklediğini söylüyor olabilir; URL ile deneyelim.
            result = vision_client.analyze(
                image_url=blob_url,
                visual_features=[VisualFeatures.READ]
            )
