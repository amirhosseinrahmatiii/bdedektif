...
# Health check (Azure App Service burayı yokluyor)
@app.get("/api/health")
def health():
...
def run_ocr(vision_client: ImageAnalysisClient, blob_url: str, image_bytes: bytes):
    """
    Sürüm farklarından etkilenmeyen güvenli yol:
    1) Önce bayt ile pozisyonel çağrı (en geniş uyumluluk)
    2) Olmazsa (çok eski varyant) URL ile dener
    """
    try:
        # En uyumlu imza: analyze(image_data_bytes, [VisualFeatures.READ])
        return vision_client.analyze(image_bytes, [VisualFeatures.READ])
    except TypeError:
        # Bazı çok eski varyantlarda analyze_from_url mevcut
